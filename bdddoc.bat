...
"%DIR%lib\Nant\nant.exe" /f:.\build.custom\bdddoc.test.step  -D:build.config.settings=%build.config.settings% -D:app.bdddoc=%APP_BDDDOC% -D:test_assembly=%TEST_ASSEMBLY_NAME%
"%DIR%lib\Nant\nant.exe" /f:.\build.custom\bdddoc.test.step open_results